...
response = client.messages.create(
    ...
    output_config={"effort": "high"},  # low / medium / high；Opus 5、Opus 4.8/4.7 还可用 xhigh / max
...
*   **极端复杂任务/Agentic 编码**：`effort: "xhigh"` 或 `"max"`（Opus 5、Opus 4.8 / 4.7 可用），在工具使用密集的场景下提供更充分的推理。官方默认档位仍是 `high`；使用 `xhigh`/`max` 时建议将 `max_tokens` 设为 64k 以上，为思考和工具调用留出充足空间。
