...
下图展示了一次请求如何依次穿过「输入 — 模型 — 输出 / 行为」三道护栏：

```mermaid
graph LR
    U["用户输入"] --> IG["输入护栏<br/>提示注入检测 · PII 过滤"]
    IG --> M(("Claude 模型"))
    M --> OG["输出护栏<br/>话题阻断 · 格式校验"]
    OG --> R["返回用户"]
    M --> BG["行为护栏<br/>权限控制 · 确认模式"]
    BG --> ACT["执行动作 / 调用工具"]
```
...
*   **防御**: 使用 Presidio 或正则表达式，在入参阶段检测并替换敏感模式。
...
*   **实现**: 运行第二个轻量级 Claude Haiku 4.5 作为 **Admin**。
