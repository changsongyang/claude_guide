...
Claude Opus 5、Sonnet 5、Fable 5、Opus 4.8、Opus 4.7 和 Opus 4.6 已进入 1 百万 Token 上下文窗口 (Context Window) 的公开规格或 beta 规格；Sonnet 4.6 仅作为迁移期兼容性基线。可用性、账号权限和计费方式必须按官方模型页、迁移指南与价格页核验。按官方价格页，这些 1M 模型的整个上下文窗口按**标准单价**计费——90 万 token 的请求与 9 千 token 的请求单价相同，缓存与批处理折扣亦全程适用。Haiku 4.5、Sonnet 4.5 等其他 Claude 模型通常为 200K。这并不意味着应该无限制地往里塞东西。
**长上下文腐烂 (Context Rot)** 和 **注意力稀释 (Attention Dilution)** 是真实存在的问题。而且，越长的 Context 意味着越慢的响应速度和更高的延迟。
...
def chat_with_memory(user_message, conversation_id):
    ...
    # 存储新对话（chromadb 的 add() 要求显式提供 ids）
    import uuid
...
# 带自定义重点的压缩（参数就是自由文本的摘要指令，没有专门的命令行旗标）
> /compact 重点保留架构决策与测试结果
```

Claude Code 的压缩策略遵循“缓存安全”原则（详见 [10.2.9 节](10.2_caching.md)）：压缩请求复用完整的系统前缀以命中提示词缓存，只有新增的压缩指令本身按全价计费。
