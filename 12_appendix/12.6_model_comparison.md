...
> **关于当前口径**：Fable 5 已于 2026-07-01 恢复全球访问；Mythos 5 非普遍可用，仅向 Project Glasswing 获批客户有限开放，采用邀请制且无自助注册。新项目先评测 Sonnet 5（`claude-sonnet-5`）；Sonnet 4.6 只作为既有系统灰度迁移与回退基线。下方历史对比表不是生产路由，实际采购必须回到官方模型/价格页和自己的评测集。
...
| 厂商 | 模型系列 | 当前最新版本 | 发布时间 |
|-----|--------|-----------|--------|
| Anthropic | Claude | Fable 5；Opus 5；Sonnet 5；Haiku 4.5 | 2026-07 快照 |
| OpenAI | GPT | GPT-5.6 Sol / Terra / Luna（GPT-5 于 2025-08 首发） | 2026-07 |
...
注：本对比重点关注 Claude、GPT、Gemini 等主流商业 LLM。由于竞品版本、价格和公开评测变化很快，表格中的非 Claude 条目只作为分析模板；实际选型应把最新官方价格页、模型卡和你自己的业务评估集作为最终依据。下文各对比表的 GPT 列停留在 GPT-5.4／GPT-5.5 一代的快照；OpenAI 已于 2026-07-09 发布 GPT-5.6 Sol / Terra / Luna 接替前沿位置（`gpt-5.6` 别名指向 Sol），这一代未纳入下表，最新对比需读者按官方 Pricing 页重新核验。
...
| **上下文长度** | 200K token | 1M token | 1M token | 1M token | 1M token | 128K token |
...
成本计算基础：标准任务（输入 500token，输出 200token）
...
需求 3: 成本敏感度
  ...
  │  └─ Claude Haiku 4.5 ✓
  ├─ 中等敏感 (性价比重要)
  │  └─ Claude Sonnet 5 或 Gemini 3.1 Pro
  └─ 成本不是主要考虑
     └─ 选择最高性能的
...
推荐：**Claude Sonnet 5**。既有 Sonnet 4.6 系统在完成业务评测、token 重计数和成本回归前保持原路由。
...
成本：按 Sonnet 5 的实际 token 数与当前价格计算；新 tokenizer 不能沿用 Sonnet 4.6 的 token 估算
质量：以内容平台自己的人工盲评和任务成功率为准
应用：博客生成、广告文案、创意写作
...
推荐：**Claude Opus 5 + Adaptive Thinking**
...
推荐：**Claude Sonnet 5**。既有 Sonnet 4.6 代码链路只在灰度迁移和回退期间保留。

特点：
- 在项目自己的代码评测集上比较正确性、可运行性和维护成本
- 重新测量新 tokenizer 带来的 token、延迟和 `max_tokens` 变化
- 默认 Adaptive Thinking；移除手动 thinking budget 和非默认 sampling 参数
...
推荐：新项目评测 **Claude Sonnet 5** 或 **GPT-5.4**；既有 Sonnet 4.6 仅作为迁移基线。

| 历史快照对比维度 | Claude Sonnet 4.6 | GPT-5.4 |
|---------|-----------|-----------|
| 数学推理 | 89.2% | 92.1% |
| 论文理解 | 优秀 | 优秀 |
...
推荐：**Claude Haiku 4.5**（主要）+**Claude Sonnet 5**（关键请求候选）。既有 Sonnet 4.6 仅用于灰度迁移与回退。

```python
# 多层级路由策略
def select_model(priority: str) -> str:
    if priority == "critical":
        return "claude-sonnet-5"  # 关键请求先经业务评测
    else:
...
迁移后成本（Claude Sonnet 5，2026-08-31 前介绍价）:
  输入: 1000 万 × 500 token × $2/M = $10,000
  输出: 1000 万 × 200 token × $10/M = $20,000
  总计: $30,000/月

  介绍价结束后按 $3/$15 计算为 $45,000/月
  注意：500/200 必须是 Sonnet 5 重新计数后的平均 token，不能复用 Sonnet 4.6 的统计
...
#### 历史快照严格 2D Pareto 前沿与实用候选

基于历史输入成本和综合性能评分（MATH + HE + MMLU 的平均值），以下区分当时的严格 2D Pareto 前沿模型与生态内实用候选。若只看“更便宜且更高分”两个维度，Sonnet 4.6 不在严格前沿上；它在本表中的位置仅用于解释历史评估方法，不是当前新项目推荐。新项目应另行评测 Sonnet 5；既有 Sonnet 4.6 系统按迁移清单灰度升级。当前 Opus 选型也应评测 Opus 4.8，不能把历史 4.7 分数当作 4.8 官方 benchmark。
...
| **Claude Sonnet 4.6（历史快照）**| $3.00 | 92 | 当时的 Claude 生态通用候选 | 实用候选（非严格 2D 前沿） |
...
Sonnet 5（介绍价）: ($2 × 500 + $10 × 200) / 1M = $0.0030
Sonnet 4.6 / Sonnet 5 标准价: ($3 × 500 + $15 × 200) / 1M = $0.0045
...
Sonnet 5（介绍价）:
  写入: 10000 × 1.25 × $2/M = $0.0250
  读取: 5 × 10000 × 0.1 × $2/M = $0.0100
  小计: $0.0350 (平均每次 $0.00583)
...
graph TD
    ...
    C["预算 $100-$1000/月"]
    C1["优先级: 成本 ≈ 性能"]
    C2["推荐: Claude Sonnet 5"]
    C3["用途: 通用应用、内容生成、代码助手"]
    C4["吞吐: 20K-100K 请求/月"]

    D["预算 $1000-$5000/月"]
    D1["优先级: 性能 > 成本"]
    D2["推荐: Sonnet 5 + 少量 Opus 5"]
    D3["用途: 混合工作流"]
    D4["吞吐: 5K-20K 请求/月"]
...
这些维度解释了 Sonnet 4.6 在该历史快照中的实用候选位置；当前新项目应评测 Sonnet 5，既有 Sonnet 4.6 只在灰度迁移与回退期间保留。
```

#### 5.4 推荐使用原则

1. **新项目先评测 Claude Sonnet 5**：既有 Sonnet 4.6 系统按迁移清单灰度升级，不用“适应 80%”这类无来源比例替代业务评测
2. **成本极限选 Haiku 4.5**：降低成本 70%，性能下降 15%，Pareto 前沿左端
3. **超高质量选 Opus 5**：当前 Opus 档型号，成本较高，适合最难任务
4. **多模态优先 Gemini 3.1 Pro**：视频理解与搜索整合强，1M 上下文，成本相对较低
5. **混合策略最优**：
   - 高频日常任务用 Haiku（70% 流量）
   - 关键任务用 Sonnet 5（25% 流量；既有 Sonnet 4.6 仅用于灰度迁移与回退）
   - 超高精度用 Opus 5（5% 流量）
