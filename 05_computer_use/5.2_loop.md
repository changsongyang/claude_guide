...
在 Computer Use 模式下，Claude 通过版本化工具定义操作桌面环境。Computer Use 仍是 beta 能力，API 请求需要按官方文档添加对应的 `anthropic-beta` 头，并使用与模型匹配的工具版本；新项目应直接复制当前官方 quickstart 中的 `computer_*`、`text_editor_*`、`bash_*` 版本字符串。

近期 Claude Sonnet 5、Claude Opus 4.8/4.7/4.6，以及作为兼容性基线的 Claude Sonnet 4.6 和 Claude Opus 4.5，其 Computer Use 组合通常是 `computer-use-2025-11-24` / `computer_20251124`，并配合 `text_editor_20250728` 与 `bash_20250124`；Claude Sonnet 4.5 / Haiku 4.5（以及已在 Claude API 退役、仅合作方云仍可用的 Opus 4.1、Sonnet 4、Opus 4）目前仍需使用 `computer-use-2025-01-24` / `computer_20250124`，并非过时组合（更早的 Claude 3.7 文档亦用此组合）。版本会随模型更新，不能写死成永久规则。

#### `computer`

这是最核心的工具，用于执行键鼠操作。
*   **输入参数**:
    *   `action`: 动作类型。基础动作包括 `screenshot`, `left_click`, `type`, `key`, `mouse_move`；较新工具还支持 `scroll`, `left_click_drag`, `right_click`, `middle_click`, `double_click`, `triple_click`, `left_mouse_down`, `left_mouse_up`, `hold_key`, `wait`，`computer_20251124` 可通过 `enable_zoom` 启用 `zoom`。
